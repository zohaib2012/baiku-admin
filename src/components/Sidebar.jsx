...
export default function Sidebar() {
  ...
  return (
    <aside className={`${collapsed ? 'w-[72px]' : 'w-60'} min-h-screen bg-gradient-to-b from-[#0F0B1A] via-[#16102B] to-[#0F0B1A] border-r border-indigo-900/20 flex flex-col shrink-0 relative transition-all duration-300`}>
        ...
        {links.map(({ to, label, icon }) => {
          ...
          return (
            ...
            >
              ...
              <span className={`relative z-10 transition-all duration-200 ${active ? 'text-indigo-300' : 'text-indigo-400/60 group-hover:text-indigo-300'}`}>
                {icon}
              </span>
              {!collapsed && (
              ...
              {active && !collapsed && (
                <span className="ml-auto relative z-10">
                  <span className="block w-1.5 h-1.5 bg-indigo-400 rounded-full shadow-sm shadow-indigo-400/50" />
                </span>
              )}
              {active && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-gradient-to-b from-indigo-400 to-violet-500 rounded-full shadow-sm shadow-indigo-400/50" />
              )}
            </NavLink>
          );
        })}
