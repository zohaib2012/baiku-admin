    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        {STATUSES.map(s => (
          ...
          >
            {s.replace(/_/g, ' ')}
            {filter === s && rides.length > 0 && (
              <span className="ml-1.5 bg-white/20 px-1.5 py-0.5 rounded text-[10px]">{rides.length}</span>
            )}
          </button>
        ))}
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-indigo-50/50 text-gray-500 text-xs uppercase tracking-wider">
              <th className="px-5 py-3.5 text-left font-semibold">Passenger</th>
              ...
              <th className="px-5 py-3.5 text-left font-semibold">Dist.</th>
            ...
            {rides.length === 0 ? (
              ...
              rides.map(ride => (
                <tr key={ride.id} className="hover:bg-indigo-50/30 cursor-pointer transition-colors" onClick={() => navigate(`/rides/${ride.id}`)}>
                  <td className="px-5 py-3.5 font-medium text-gray-800">{ride.passengerName || '—'}</td>
                  <td className="px-5 py-3.5 text-gray-500">{ride.driverName || <span className="text-gray-300 italic">Not assigned</span>}</td>
                  ...
                  <td className="px-5 py-3.5 font-medium">
                    <span className="bg-indigo-50 px-2 py-1 rounded-lg text-xs text-indigo-700 font-semibold">
                      Rs. {ride.finalPrice || ride.passengerPrice || '—'}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-gray-500 text-xs">{ride.distance?.toFixed(1)} km</td>
                  <td className="px-5 py-3.5" onClick={e => e.stopPropagation()}><Badge status={ride.status} /></td>
                  ...
                  <td className="px-5 py-3.5" onClick={e => e.stopPropagation()}>
                    {['searching', 'negotiating', 'confirmed', 'active'].includes(ride.status) && (
                      <button
                        onClick={() => setCancelTarget(ride.id)}
      ...
      <ConfirmModal
        ...
        danger
        onConfirm={() => cancelRide(cancelTarget)}
