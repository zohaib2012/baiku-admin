...
export default function PendingDrivers() {
  ...
  return (
    <div className="space-y-6">
      ...
      {drivers.length === 0 ? (
        <div className="bg-white rounded-2xl border border-indigo-100/30 p-16 text-center shadow-sm">
        ...
        <div className="grid gap-4">
          {drivers.map((driver, i) => {
            ...
            return (
              <div key={driver.id} className="bg-white rounded-2xl border border-indigo-100/30 shadow-sm hover:shadow-md transition-all duration-200 p-6 slide-up" style={{ animationDelay: `${i * 80}ms` }}>
                <div className="flex items-start justify-between flex-wrap gap-4">
                  <div className="flex items-center gap-4">
                    {driver.photoUrl ? (
                      <img src={driver.photoUrl} alt={driver.name} className="w-14 h-14 rounded-full object-cover ring-2 ring-indigo-100" />
                    ) : (
                      <div className="w-14 h-14 rounded-full bg-gradient-to-br from-indigo-50 to-violet-100 flex items-center justify-center text-2xl ring-2 ring-indigo-100">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-4 py-2 text-sm border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all font-medium text-gray-700"
                    >
                      View Details
                    </button>
                    <button
                      ...
                      className="px-4 py-2 text-sm bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl hover:from-emerald-600 hover:to-emerald-700 transition-all font-medium disabled:opacity-60 shadow-sm hover:shadow-md flex items-center gap-1.5"
                    >
                      {isActionLoading ? (
                    ...
                    <button
                      ...
                      disabled={isActionLoading}
                      className="px-4 py-2 text-sm bg-gradient-to-r from-rose-500 to-rose-600 text-white rounded-xl hover:from-rose-600 hover:to-rose-700 transition-all font-medium disabled:opacity-60 shadow-sm hover:shadow-md flex items-center gap-1.5"
                    >
                      {isActionLoading ? (
                        <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
                      ...
                      {url ? (
                        <a href={url} target="_blank" rel="noreferrer">
                          <img
                            src={url}
                            alt={label}
                            className="w-full h-20 object-cover rounded-xl border border-indigo-100/30 hover:opacity-80 hover:shadow-md transition-all cursor-pointer"
                          />
                        </a>
                      ) : (
