        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
          {docs.map(({ label, url }) => (
            <div key={label}>
              ...
              {url ? (
                <img src={url} alt={label} onClick={() => setModalImg(url)}
                  className="w-full h-24 sm:h-28 object-cover rounded-xl border border-indigo-100/30 cursor-pointer hover:opacity-80 hover:shadow-md transition-all" />
              ) : (
